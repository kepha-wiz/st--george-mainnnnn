...
class LoginTestCase(unittest.TestCase):
    ...
    def test_accept_enrollment_request(self):
        driver = self.driver
        ...
        # Log in as a random admin
        random_admin = User.query.filter_by(user_type='admin').order_by(func.random()).first()
        username_field = driver.find_element(By.ID, "username")
        username_field.send_keys(random_admin.username)
        password_field = driver.find_element(By.ID, "password")
        password_field.send_keys(random_admin.password)
        login_button = driver.find_element(By.ID, "submit")
        login_button.click()

        # Click on the "Accept and Decline Student Enrollment Requests for Courses" button
        enrollment_requests_button = driver.find_element(By.ID, "enrollment_requests_button")
        enrollment_requests_button.click()

        # Click on the "Accept" button under the first course request box
        accept_button = driver.find_element(By.ID, "accept_button")
        accept_button.click()

        # Query the database to check if the accepted course appears under enrollments section
        accepted_course = Enrollment.query.filter_by(course_id=1).first()  # Assuming the first course has id=1
        self.assertIsNotNone(accepted_course, "Accepted course should appear under enrollments section")
